...
import {
  Beaker, Apple, Carrot, Wheat, Fish, Droplet, Plus, Trash2, Download,
  Users, User, BookOpen, ChevronRight, AlertCircle, Sparkles, Leaf,
  FlaskConical, ScrollText, ArrowUpRight
} from 'lucide-react';

// =============================================================
// LOCAL STORAGE WRAPPER
// Mimics the window.storage API used in Claude artifacts so the
// rest of the component code is unchanged. Data persists in the
// current browser only — for true multi-participant cohort use,
// replace this with a backend (Supabase, Firebase, etc.).
// =============================================================
const localStore = {
  async list(prefix) {
    const keys = [];
...
// =============================================================
// EMBEDDED FOOD-PIGMENT REFERENCE DATASET
// Values: mg per 100g edible portion
// Sources: USDA Database for the Flavonoid Content of Selected
// Foods, Release 3.3 (2018) + Khoo et al. 2017 Table 1
// =============================================================

const FLAVONOID_CLASSES = {
  anthocyanidins: { label: 'Anthocyanidins', color: '#6B2D5C', compounds: ['Cyanidin','Delphinidin','Malvidin','Pelargonidin','Peonidin','Petunidin'] },
  flavan3ols:    { label: 'Flavan-3-ols',    color: '#8B5A3C', compounds: ['(-)-Epicatechin','(-)-Epicatechin 3-gallate','(-)-Epigallocatechin','(-)-Epigallocatechin 3-gallate','(+)-Catechin','(+)-Gallocatechin','Theaflavin','Theaflavin-3,3\'-digallate','Theaflavin-3\'-gallate','Theaflavin-3-gallate','Thearubigins'] },
  flavanones:    { label: 'Flavanones',      color: '#E8A33D', compounds: ['Eriodictyol','Hesperetin','Naringenin'] },
  flavones:      { label: 'Flavones',        color: '#A8B83A', compounds: ['Apigenin','Luteolin'] },
  flavonols:     { label: 'Flavonols',       color: '#D4A017', compounds: ['Isorhamnetin','Kaempferol','Myricetin','Quercetin'] },
  isoflavones:   { label: 'Isoflavones',     color: '#C97B5C', compounds: ['Daidzein','Genistein','Glycitein'] },
};

const COLOR_CATEGORIES = {
  red:        { label: 'Red',          hex: '#B33A3A' },
  orangeYellow:{ label: 'Orange/Yellow',hex: '#E8A33D' },
  green:      { label: 'Green',        hex: '#5A8A3A' },
  bluePurple: { label: 'Blue/Purple',  hex: '#6B2D5C' },
  whiteTan:   { label: 'White/Tan',    hex: '#C9B98F' },
};

// Food database — abbreviated; flavonoid values in mg/100g
const FOOD_DB = [
...
// Approximate U.S. NHANES 2007-2008 mean intakes (mg/day, adults 19+)
const NHANES_MEAN = {
  anthocyanidins: 11.6, flavan3ols: 156.5, flavanones: 14.4,
  flavones: 1.6, flavonols: 12.9, isoflavones: 1.3,
};

// Khoo et al. 2017 — Tables 2 & 3 health benefit linkages (paraphrased)
...
export default function ChromaDiet() {
  ...
  // Load cohort data on mount (and after each new submission)
  useEffect(() => {
    (async () => {
      try {
  ...
  const analysis = useMemo(() => {
    ...
    const anthoScore = Math.min(flavTotals.anthocyanidins / 25, 1) * 10; // Khoo: ~25mg/day threshold for benefits
  ...
  const clearCohort = async () => {
    if (!confirm('Clear all locally stored cohort submissions? This cannot be undone.')) return;
    try {
      const list = await localStore.list('chromadiet:participant:');
  ...
  const TabButton = ({ id, label, icon: Icon }) => (
    <button
      onClick={() => setTab(id)}
      className={`flex items-center gap-2 px-5 py-3 text-sm tracking-wide transition-all border-b-2 ${
        tab === id ? 'border-stone-900 text-stone-900 font-medium' : 'border-transparent text-stone-500 hover:text-stone-800'
      }`}
    >
      <Icon size={15} strokeWidth={1.5} />
      <span style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: '15px', letterSpacing: '0.05em' }}>{label}</span>
    </button>
  );

  const Card = ({ children, className = '', accent }) => (
    <div className={`bg-white border border-stone-200 ${className}`} style={accent ? { borderTop: `3px solid ${accent}` } : {}}>
      {children}
    </div>
  );
  ...
  const SectionHeading = ({ eyebrow, title, sub }) => (
    <div className="mb-8 pb-6 border-b border-stone-200">
      {eyebrow && <div className="text-[10px] tracking-[0.3em] uppercase text-stone-500 mb-3">{eyebrow}</div>}
      <h2 style={{ fontFamily: 'Cormorant Garamond, Georgia, serif' }} className="text-4xl text-stone-900 leading-tight">{title}</h2>
      {sub && <p className="text-stone-600 mt-2 text-sm leading-relaxed max-w-2xl">{sub}</p>}
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-stone-900" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@300;400;500;600;700&family=Inter:wght@300;400;500;600&family=JetBrains+Mono:wght@400;500&display=swap" />
        ...
        <div className="max-w-7xl mx-auto px-8 py-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{
              background: 'conic-gradient(from 0deg, #6B2D5C, #8B5A3C, #E8A33D, #A8B83A, #D4A017, #C97B5C, #6B2D5C)'
            }} />
            <div>
              <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl font-medium tracking-tight leading-none">ChromaDiet</div>
              <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mt-1">Food Color Pigment & Flavonoid Intake Analyzer</div>
            </div>
          </div>
          <div className="text-right">
            <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500">Participant</div>
            <div style={{ fontFamily: 'JetBrains Mono, monospace' }} className="text-sm">{participantId}</div>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-8 py-12">
        ...
        {tab === 'intake' && (
          <div>
            ...
            <Card className="p-6 mb-6">
              <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-4">Onboarding</div>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  <label className="block text-xs text-stone-600 mb-1">Age range</label>
                  <select value={demographics.ageRange} onChange={e=>setDemographics({...demographics, ageRange:e.target.value})} className="w-full border border-stone-300 px-3 py-2 text-sm bg-white">
                ...
                <div>
                  ...
                  <select value={demographics.sex} onChange={e=>setDemographics({...demographics, sex:e.target.value})} className="w-full border border-stone-300 px-3 py-2 text-sm bg-white">
                    <option value="">Select</option>
                    <option>Female</option><option>Male</option><option>Other / Prefer not to say</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs text-stone-600 mb-1">Study / Cohort code</label>
                  <input value={demographics.cohortCode} onChange={e=>setDemographics({...demographics, cohortCode:e.target.value})} placeholder="e.g., NUTR-2026-A" className="w-full border border-stone-300 px-3 py-2 text-sm" />
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <Card className="p-5">
                <div className="flex items-center gap-2 text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-3"><ScrollText size={12} /> Text template</div>
                <div style={{ fontFamily: 'JetBrains Mono, monospace' }} className="text-xs bg-stone-50 p-3 border border-stone-200 leading-relaxed">
              ...
              <Card className="p-5">
                <div className="flex items-center gap-2 text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-3"><Apple size={12} /> Image upload guidance</div>
                <p className="text-xs text-stone-700 leading-relaxed">
                  Photograph each meal from above with a reference object (e.g., a fork or coin) for portion estimation.
                  Capture beverages in their original container when possible.
                  <span className="block mt-2 italic text-stone-500">Image OCR / vision integration is supported via API; pre-loaded reference dataset shown below.</span>
                </p>
              </Card>
            </div>
            ...
            <Card>
              <div className="p-5 flex items-center justify-between border-b border-stone-200">
                <div>
                  <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl">Food Entries</div>
                  <div className="text-xs text-stone-500 mt-1">{entries.length} item{entries.length !== 1 ? 's' : ''} logged</div>
                </div>
                <button onClick={addEntry} className="flex items-center gap-2 bg-stone-900 text-white px-4 py-2 text-xs tracking-widest uppercase hover:bg-stone-700">
              ...
              <div className="divide-y divide-stone-100">
                {entries.map(e => {
                  ...
                  return (
                    <div key={e.id} className="p-4 grid grid-cols-12 gap-3 items-center hover:bg-stone-50">
                      <div className="col-span-1 flex justify-center">
                        {cat && <div className="w-3 h-3 rounded-full" style={{ background: cat.hex }} title={cat.label} />}
                      </div>
                      <div className="col-span-5">
                        <select value={e.foodId} onChange={ev=>updateEntry(e.id,'foodId',ev.target.value)} className="w-full border border-stone-200 px-3 py-2 text-sm bg-white">
                          {FOOD_DB.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}
                        </select>
                      </div>
                      <div className="col-span-3">
                        <div className="flex items-center gap-2">
                          <input type="number" min="0" value={e.portionG} onChange={ev=>updateEntry(e.id,'portionG',Number(ev.target.value))} className="w-full border border-stone-200 px-3 py-2 text-sm" />
                          <span className="text-xs text-stone-500">g/ml</span>
                        </div>
                      </div>
                      <div className="col-span-2">
                        <select value={e.meal} onChange={ev=>updateEntry(e.id,'meal',ev.target.value)} className="w-full border border-stone-200 px-3 py-2 text-sm bg-white">
                          <option>Breakfast</option><option>Lunch</option><option>Dinner</option><option>Snack</option>
                        </select>
                      </div>
                      <div className="col-span-1 flex justify-end">
                        <button onClick={()=>removeEntry(e.id)} className="text-stone-400 hover:text-red-600 p-1">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </div>
              ...
              <div className="p-5 bg-stone-50 border-t border-stone-200 flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex gap-3">
                  <button onClick={()=>setTab('results')} className="px-5 py-2 text-xs tracking-widest uppercase border border-stone-900 text-stone-900 hover:bg-stone-900 hover:text-white">View Results <ChevronRight size={12} className="inline ml-1" /></button>
                  <button onClick={submitToCohort} className="px-5 py-2 text-xs tracking-widest uppercase bg-stone-900 text-white hover:bg-stone-700">{submitted ? '✓ Submitted' : 'Submit to Cohort'}</button>
                </div>
              </div>
            </Card>
          </div>
        ...
        {tab === 'results' && (
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <Card className="p-6" accent="#6B2D5C">
                <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500">ChromaDiet Score</div>
                <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-6xl font-light mt-2">{analysis.totalScore}<span className="text-2xl text-stone-400">/100</span></div>
                <div className="text-xs text-stone-600 mt-2">Composite evidence-based score</div>
              </Card>
              <Card className="p-6" accent="#8B5A3C">
                <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500">Total Flavonoids</div>
                <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-6xl font-light mt-2">{analysis.totalMg.toFixed(0)}<span className="text-2xl text-stone-400">mg</span></div>
                <div className="text-xs text-stone-600 mt-2">vs. NHANES mean ~198 mg/day</div>
              </Card>
              <Card className="p-6" accent="#A8B83A">
                <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500">Classes Present</div>
                <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-6xl font-light mt-2">{analysis.classesPresent}<span className="text-2xl text-stone-400">/6</span></div>
                <div className="text-xs text-stone-600 mt-2">USDA flavonoid classes</div>
              </Card>
              <Card className="p-6" accent="#E8A33D">
                <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500">Color Categories</div>
                <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-6xl font-light mt-2">{analysis.colorsPresent}<span className="text-2xl text-stone-400">/5</span></div>
            ...
            {/* MyPlate row */}
            <Card className="p-6 mb-6">
              <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-4">MyPlate Coverage / U.S. Dietary Guidelines</div>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {[
                  { key:'fruits', label:'Fruits', icon:Apple, color:'#B33A3A' },
                  { key:'vegetables', label:'Vegetables', icon:Carrot, color:'#5A8A3A' },
                  { key:'wholeGrains', label:'Whole Grains', icon:Wheat, color:'#C9B98F' },
                  { key:'protein', label:'Protein', icon:Fish, color:'#8B5A3C' },
                  ...
                  return (
                    <div key={g.key} className="border border-stone-200 p-4">
                      ...
                      <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-3xl mt-2">{count}</div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
              <Card className="p-6">
                <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-1">Intake by Flavonoid Class</div>
                <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl mb-4">Six classes, six colors</div>
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={classBarData} layout="vertical" margin={{ left: 20 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#e7e5e4" />
                    <XAxis type="number" tick={{ fontSize: 11 }} />
                    <YAxis type="category" dataKey="name" tick={{ fontSize: 11 }} width={110} />
                    <Tooltip formatter={(v) => `${v} mg`} />
                    <Bar dataKey="mg" radius={[0, 2, 2, 0]}>
              ...
              <Card className="p-6">
                <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-1">vs. U.S. National Average</div>
                <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl mb-4">NHANES 2007–2008 comparison</div>
                <ResponsiveContainer width="100%" height={280}>
                  <RadarChart data={radarData}>
                    <PolarGrid stroke="#d6d3d1" />
                    <PolarAngleAxis dataKey="class" tick={{ fontSize: 10 }} />
                    <PolarRadiusAxis tick={{ fontSize: 9 }} />
                    <Radar name="You" dataKey="intake" stroke="#6B2D5C" fill="#6B2D5C" fillOpacity={0.4} />
                    <Radar name="NHANES Mean" dataKey="nhanes" stroke="#78716c" fill="#78716c" fillOpacity={0.15} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
              <Card className="p-6">
                <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-1">Pigment Distribution by Color</div>
                <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl mb-4">A spectrum on your plate</div>
                {colorPieData.length ? (
                  <ResponsiveContainer width="100%" height={280}>
                    <PieChart>
              ...
              <Card className="p-6">
                <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-1">By Meal Occasion</div>
                <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl mb-4">When you got your pigments</div>
                <ResponsiveContainer width="100%" height={280}>
            ...
            {/* Fruit & vegetable breakdown */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <Card className="p-6">
                <div className="flex items-center gap-3 mb-4">
                  <Apple size={20} className="text-red-700" strokeWidth={1.5} />
                  <div>
                    <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500">Fruits Logged</div>
                    <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-3xl">{analysis.fruitCount}</div>
                  </div>
                </div>
                <div className="space-y-2">
                  {Object.keys(COLOR_CATEGORIES).map(c => (
                    <div key={c} className="flex items-center gap-3 text-sm">
                      ...
                      <div className="font-mono text-stone-900">{analysis.fruitColorCounts[c]}</div>
                    </div>
                  ))}
                </div>
              </Card>
              <Card className="p-6">
                <div className="flex items-center gap-3 mb-4">
                  <Carrot size={20} className="text-green-700" strokeWidth={1.5} />
                  <div>
                    <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500">Vegetables Logged</div>
                    <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-3xl">{analysis.vegCount}</div>
                  </div>
                </div>
                <div className="space-y-2">
                  {Object.keys(COLOR_CATEGORIES).map(c => (
                    <div key={c} className="flex items-center gap-3 text-sm">
                      <div className="w-3 h-3 rounded-full" style={{ background: COLOR_CATEGORIES[c].hex }} />
                      <div className="flex-1 text-stone-700">{COLOR_CATEGORIES[c].label}</div>
                      <div className="font-mono text-stone-900">{analysis.vegColorCounts[c]}</div>
                    </div>
                  ))}
                </div>
              </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
              <Card className="p-6">
                <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-1">Projected Health Benefit Alignment</div>
                <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl mb-4">From Khoo et al. 2017, Tables 2–3</div>
                    ...
                    return (
                      <div key={k} className="border-l-2 pl-4 py-1" style={{ borderColor: FLAVONOID_CLASSES[k].color }}>
                        <div className="flex items-baseline justify-between">
                          ...
                          <div className="text-xs text-stone-500 font-mono">{mg.toFixed(1)} mg</div>
                        </div>
              ...
              <Card className="p-6 bg-amber-50 border-amber-200">
                <div className="flex items-center gap-2 text-[10px] tracking-[0.25em] uppercase text-amber-700 mb-1"><AlertCircle size={12} /> Gaps & Opportunities</div>
                <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl mb-4">Missing color pigments</div>
                {analysis.missingClasses.length > 0 && (
                  <div className="mb-4">
                    <div className="text-xs uppercase tracking-wide text-stone-600 mb-2">Flavonoid classes absent today</div>
                    <div className="flex flex-wrap gap-2">
                      {analysis.missingClasses.map(c => (
                        <div key={c} className="px-3 py-1 text-xs border border-stone-300 bg-white">{FLAVONOID_CLASSES[c].label}</div>
                      ))}
                    </div>
                  </div>
                )}
                {analysis.missingColors.length > 0 && (
                  <div>
                    <div className="text-xs uppercase tracking-wide text-stone-600 mb-2">Color categories absent today</div>
                    <div className="flex flex-wrap gap-2">
                      {analysis.missingColors.map(c => (
                        <div key={c} className="px-3 py-1 text-xs border border-stone-300 bg-white flex items-center gap-2">
                ...
                {analysis.missingClasses.length === 0 && analysis.missingColors.length === 0 && (
                  <div className="text-sm text-stone-700">Excellent — all six flavonoid classes and all five color categories are represented in your 24-hour recall.</div>
                )}
              </Card>
            </div>
            ...
            {/* Score breakdown */}
            <Card className="p-6 mb-6">
              <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-1">About Your ChromaDiet Score</div>
              <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl mb-4">Transparent algorithm</div>
              <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                {[
                  ...
                  <div key={s.label} className="border border-stone-200 p-3">
                    <div className="text-xs text-stone-600">{s.label}</div>
                    <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl">{s.val.toFixed(1)}<span className="text-sm text-stone-400">/{s.max}</span></div>
            ...
            <div className="flex flex-wrap gap-3 justify-end">
              <button onClick={exportReport} className="flex items-center gap-2 px-5 py-2 text-xs tracking-widest uppercase border border-stone-900 text-stone-900 hover:bg-stone-900 hover:text-white">
        ...
        {tab === 'cohort' && (
          <div>
            ...
            />
            {cohortLoading ? (
              <div className="text-stone-500 text-sm">Loading cohort data…</div>
            ) : !cohortStats ? (
              <Card className="p-12 text-center">
                <Users size={32} strokeWidth={1.2} className="mx-auto text-stone-400 mb-4" />
                <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl mb-2">No submissions yet</div>
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <Card className="p-6" accent="#6B2D5C">
                    <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500">Participants</div>
                    <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-6xl font-light mt-2">{cohortStats.n}</div>
                  </Card>
                  <Card className="p-6" accent="#A8B83A">
                    <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500">Mean ChromaDiet Score</div>
                    <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-6xl font-light mt-2">{cohortStats.meanScore.toFixed(1)}<span className="text-2xl text-stone-400">/100</span></div>
                  </Card>
                  <Card className="p-6" accent="#E8A33D">
                    <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500">Mean Total Flavonoids</div>
                    <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-6xl font-light mt-2">{Object.values(cohortStats.means).reduce((a,b)=>a+b,0).toFixed(0)}<span className="text-2xl text-stone-400">mg</span></div>
                  </Card>
                </div>

                <Card className="p-6 mb-6">
                  <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-1">Cohort vs. NHANES National Mean</div>
                  <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl mb-4">Mean intake by flavonoid class</div>
                  <ResponsiveContainer width="100%" height={320}>
                    <BarChart data={Object.keys(FLAVONOID_CLASSES).map(k => ({
                      ...
                      <Tooltip />
                      <Legend wrapperStyle={{ fontSize: '11px' }} />
                      <Bar dataKey="Cohort" fill="#6B2D5C" />
                      <Bar dataKey="NHANES" fill="#a8a29e" />
                    </BarChart>
                  </ResponsiveContainer>
                </Card>

                <div className="flex justify-end">
                  <button onClick={clearCohort} className="text-xs text-stone-500 hover:text-red-600 underline">Clear local cohort data</button>
                </div>
              </>
            )}
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                { name:'Texture / IDDSI Framework', desc:'Standardized food and drink texture descriptors for clinical and research applications.' },
              ].map(db => (
                <Card key={db.name} className="p-6">
                  <div className="flex items-start gap-3 mb-2">
                    <FlaskConical size={18} strokeWidth={1.5} className="text-stone-500 mt-1" />
                    <div>
                      <div style={{ fontFamily: 'Cormorant Garamond, serif' }} className="text-2xl">{db.name}</div>
                      <div className="text-[10px] tracking-[0.25em] uppercase text-amber-700 mt-1">Integration planned</div>
                    </div>
                  </div>
        ...
        {tab === 'methods' && (
          <div>
            ...
            <Card className="p-6 mb-4">
              <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-3">Primary References</div>
              <ol className="space-y-4 text-sm">
                <li className="flex gap-3">
                  <BookOpen size={16} strokeWidth={1.5} className="text-stone-500 mt-0.5 flex-shrink-0" />
                  <div>
                    <div className="font-medium">USDA Database for the Flavonoid Content of Selected Foods, Release 3.3</div>
                    <div className="text-stone-600 text-xs mt-1">Defines six flavonoid classes and 29 individual compounds. Used for all mg/100g values.</div>
                    <a href="https://www.ars.usda.gov/ARSUserFiles/80400535/Data/Flav/Flav3.3.pdf" target="_blank" rel="noreferrer" className="text-xs text-stone-900 underline inline-flex items-center gap-1 mt-1">View source <ArrowUpRight size={12} /></a>
                  </div>
                </li>
                <li className="flex gap-3">
                  <BookOpen size={16} strokeWidth={1.5} className="text-stone-500 mt-0.5 flex-shrink-0" />
                  <div>
                    ...
                    <a href="https://pmc.ncbi.nlm.nih.gov/articles/PMC5613902/" target="_blank" rel="noreferrer" className="text-xs text-stone-900 underline inline-flex items-center gap-1 mt-1">View source <ArrowUpRight size={12} /></a>
                  </div>
                </li>
                <li className="flex gap-3">
                  <BookOpen size={16} strokeWidth={1.5} className="text-stone-500 mt-0.5 flex-shrink-0" />
                  <div>
                    ...
                    <a href="https://www.ars.usda.gov/ARSUserFiles/80400530/pdf/0708/flav_tables_1-4_2007-2008.pdf" target="_blank" rel="noreferrer" className="text-xs text-stone-900 underline inline-flex items-center gap-1 mt-1">View source <ArrowUpRight size={12} /></a>
                  </div>
                </li>
                <li className="flex gap-3">
                  <BookOpen size={16} strokeWidth={1.5} className="text-stone-500 mt-0.5 flex-shrink-0" />
            ...
            <Card className="p-6 mb-4">
              <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-3">ChromaDiet Score Formula</div>
              <div style={{ fontFamily: 'JetBrains Mono, monospace' }} className="bg-stone-50 p-4 text-xs leading-relaxed border border-stone-200">
            ...
            <Card className="p-6 mb-4">
              <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-3">Limitations & Caveats</div>
              <ul className="text-sm text-stone-700 space-y-2 list-disc pl-5">
                ...
                <li>Health benefit linkages are summarized from observational/mechanistic literature and should not be interpreted as clinical guidance.</li>
            ...
            <Card className="p-6">
              <div className="text-[10px] tracking-[0.25em] uppercase text-stone-500 mb-3">Six USDA Flavonoid Classes & Their 29 Compounds</div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.keys(FLAVONOID_CLASSES).map(k => (
      ...
      <footer className="border-t border-stone-200 mt-16 py-8 px-8 max-w-7xl mx-auto">
        <div className="flex flex-wrap items-center justify-between gap-4 text-xs text-stone-500">
          <div className="flex items-center gap-2">
            <Leaf size={12} />
            <span>ChromaDiet · Research Tool · For IRB-approved studies and educational use</span>
          </div>
          <div style={{ fontFamily: 'JetBrains Mono, monospace' }}>v0.1 · Phase 1 · Standalone</div>
        </div>
      </footer>
    </div>
  );
